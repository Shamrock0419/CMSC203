   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
       ...
       for ( int row = 0; row < intArray.length; row++ )
       {
    	   if(intArray[row][column] < lowestNum) {
    		   lowestNum = intArray[row][column];
    		   animate( row, column, lowestNum); // needed to create visual feedback
    	   }
       }
       System.out.println( );  

      return lowestNum; // replace this line with your return statement
   }
