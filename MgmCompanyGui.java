...
public class MgmCompanyGui extends Application {
	...
	
	{
		...
		if (!propertyFieldsEmpty())
		{
			...
			case -2: {
				alert.setContentText( "Property "+p.getName()+" is null\n");
				alert.showAndWait();
				plotFrame.getChildren().remove(propRectangle);
				break;
			}
			case -3: {
				alert.setContentText( "Property "+p.getName()+" is not within the limits of Management Company "+mgmCompany.getName()+"\n");
				alert.showAndWait();
				plotFrame.getChildren().remove(propRectangle);
				break;
			}
			case -4: {
				alert.setContentText( "Property "+p.getName()+"'s plot overlaps another property managed by Management Company "+mgmCompany.getName()+"\n");
				alert.showAndWait();
				plotFrame.getChildren().remove(propRectangle);
				break;
			}
			default: //the case where property is valid and created - now display icon
				...
				alert.setContentText( "Property "+p.getName()+" was added\n");
				alert.showAndWait();
				break;
			}
		}
